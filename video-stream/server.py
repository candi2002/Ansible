...
def capture():
    ...
    # Kiểm tra xem im có phải là mảng NumPy không
    if im is not None and isinstance(im, np.ndarray):
        capture_and_save(im)
        logger.debug("Image captured and saved successfully.")
    else:
        logger.error("Captured image is not valid.")
        return "Error: Captured image is not valid.", 400  # Trả về mã lỗi 400
